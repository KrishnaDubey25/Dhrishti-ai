...
function Profile({p}:{p:PatientProfile}){const[f,setF]=useState(p);const complete=Boolean(f.name&&f.email&&f.phone&&f.dob&&f.gender&&f.address&&f.emergencyContact);function save(){if(!complete){alert('Complete all profile fields before saving.');return}updatePatient(p.userId,{...f,profileComplete:Math.max(85,p.profileComplete)});alert('Patient profile saved')}return <><PageTitle title="Patient profile & account" sub="Your identity, contact details and screening-account readiness in one place."/><div className="profile-premium-grid"><aside className="patient-account-card"><div className="patient-avatar">{p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase()}</div><span className="eyebrow light">Patient account</span><h2>{p.name}</h2><p>{p.email}</p><div className="account-readiness"><div><span>Account readiness</span><b>{complete?'Complete':'Needs details'}</b></div><div className="readiness-track"><i style={{width:`${complete?100:p.profileComplete}%`}}/></div></div><div className="account-facts"><span><b>Gmail identity</b><small>{p.email.endsWith('@gmail.com')?'Patient Gmail format valid':'Legacy account'}</small></span><span><b>Health Q&A</b><small>{p.onboarding?'Completed':'Pending'}</small></span><span><b>Reports</b><small>Linked to this patient ID</small></span></div></aside><section className="card premium-profile-form"><div className="section-head"><div><span className="eyebrow">Personal information</span><h3>Account details</h3></div><Status>{complete?'Ready':'Incomplete'}</Status></div><div className="field"><label>Gmail address</label><input className="input locked-input" value={f.email} readOnly/><small className="small">Email is fixed for this local patient account.</small></div><div className="grid grid-2"><F label="Full name *" v={f.name} set={v=>setF({...f,name:v})}/><F label="Phone *" v={f.phone||''} set={v=>setF({...f,phone:v})}/><F label="Date of birth *" type="date" v={f.dob||''} set={v=>setF({...f,dob:v})}/><F label="Gender *" v={f.gender||''} set={v=>setF({...f,gender:v})}/><F label="Emergency contact *" v={f.emergencyContact||''} set={v=>setF({...f,emergencyContact:v})}/></div><F label="Address / locality *" v={f.address||''} set={v=>setF({...f,address:v})}/><button className="btn primary" disabled={!complete} onClick={save}>Save complete profile</button></section></div></>}
...
function Reports({p}:{p:PatientProfile}){const d=readDB();const uploaded=p.onboarding?.uploadedReports||[];const generated=d.reports.filter(r=>r.patientId===p.id&&r.generated).sort((a,b)=>b.uploadedAt.localeCompare(a.uploadedAt));const final=d.screenings.filter(s=>s.patientId===p.id&&s.doctorReview);return <><PageTitle title="Reports" sub="Your generated screening reports, uploaded medical records and specialist-finalized reports."/><div className="grid grid-3 report-library"><div className="card"><span className="eyebrow">Generated</span><h3>Screening reports</h3>{generated.length?generated.map(x=><div className="generated-report-row" key={x.id}><div><FileText size={18}/><span><b>{x.category==='ai-screening'?'AI retinal screening report':`${x.eyeSide?`${x.eyeSide==='left'?'Left':'Right'} eye · `:''}Preliminary screening report`}</b><small>{new Date(x.uploadedAt).toLocaleString()}</small></span></div>{x.dataUrl&&<a className="btn soft mini" href={x.dataUrl} download={x.name}><FileDown size={14}/>Download</a>}</div>):<p className="muted">Complete guided eye capture to generate your first report.</p>}</div><div className="card"><span className="eyebrow">Medical context</span><h3>Uploaded records</h3>{uploaded.length?uploaded.map(x=><div className="file-row" key={x.id}><FileText size={17}/><span>{x.name}</span>{x.dataUrl&&<a href={x.dataUrl} download={x.name}>Open</a>}</div>):<p className="muted">No reports uploaded.</p>}</div><div className="card"><span className="eyebrow">Clinician finalized</span><h3>Final screening reports</h3>{final.length?final.map(s=><button key={s.id} className="btn ghost" onClick={()=>downloadFinal(p,s)}>Download {new Date(s.createdAt).toLocaleDateString()}</button>):<p className="muted">No finalized specialist report yet.</p>}</div></div></>}
function Book({p}:{p:PatientProfile}){
  const d=readDB();const[phc,setPhc]=useState(d.phcs[0]?.id||'');const[date,setDate]=useState('');const[slot,setSlot]=useState('10:00');const[report,setReport]=useState<UploadRecord|null>(null);
  ...
  function confirm(){if(!phc||!date)return;const db=readDB();if(report)db.reports.push(report);writeDB(db);addAppointment({id:uid('appt'),patientId:p.id,phcId:phc,date,slot,status:'booked',initialReportId:report?.id,createdAt:new Date().toISOString()});alert('Screening booked');setDate('')}
...
function Onboarding({p,onDone}:{p:PatientProfile;onDone:()=>void}){
  const qs=[
    ['diabetesStatus','Do you have diabetes?',['Yes','No','Not sure']],
    ['duration','How long have you had diabetes?',['Less than 1 year','1–5 years','5–10 years','More than 10 years','Not applicable']],
    ['treatment','How is diabetes currently managed?',['Lifestyle','Tablets','Insulin','Combination','Not sure','Not applicable']],
    ['medication','Are you currently taking diabetes medication?',['Yes','No','Not applicable']],
    ['previousEyeExam','Have you had a retinal/eye examination before?',['Yes','No','Not sure']],
    ['eyeProblems','Have you been told you have any eye problem?',['No known eye problem','Cataract','Glaucoma','Retina problem','Other / not sure']],
    ['symptoms','Current vision symptoms?',['None','Blurred vision','Floaters','Sudden vision change','Difficulty seeing at night','Other']],
    ['drHistory','Previous diabetic retinopathy/retina history?',['Yes','No','Not sure']],
    ['medicalContext','Relevant medical context?',['Hypertension','Kidney disease','Pregnancy','None/Other']]
  ] as const;
  const[o,setO]=useState<OnboardingData>(p.onboarding||defaults);const[step,setStep]=useState(0);const[validation,setValidation]=useState('');
  async function upload(fs:FileList|null){if(!fs)return;const arr:UploadRecord[]=[];for(const f of Array.from(fs))arr.push(await toUpload(f));setO({...o,uploadedReports:[...o.uploadedReports,...arr]})}
  const finalReady=Boolean((o.hba1c||'').trim()&&(o.systolicBP||'').trim()&&(o.cholesterol||'').trim()&&(o.currentTreatment||'').trim());
  function finish(){if(!finalReady){setValidation('Complete every clinical detail. If a value is unknown, type “Not known”. Medical report upload is the only optional item.');return}setValidation('');const risk=assessScreeningUrgency(o);updatePatient(p.userId,{onboarding:{...o,risk},profileComplete:78});onDone()}
  const q=qs[step];const progress=Math.min(100,((step+1)/(qs.length+1))*100);
  return <div className="modal-back onboarding-backdrop"><div className="modal onboarding premium-onboarding"><div className="onboarding-top"><div><span className="eyebrow">Patient health profile</span><h2>{step<qs.length?`Question ${step+1} of ${qs.length}`:'Final clinical details'}</h2><p className="muted">Health questions are required for triage. Previous medical-report upload is completely optional and never blocks the next step.</p></div><div className="required-badge"><ShieldAlert size={16}/>Questions required</div></div><div className="progress premium-progress"><div style={{width:`${progress}%`}}/></div>
    {step<qs.length?<div className="question premium-question"><span className="question-number">{String(step+1).padStart(2,'0')}</span><h3>{q[1]}</h3><div className="option-grid premium-options">{q[2].map(v=><button type="button" key={v} className={'option '+((o as any)[q[0]]===v?'selected':'')} onClick={()=>{setO({...o,[q[0]]:v});setValidation('')}}>{v}</button>)}</div></div>:<div className="question premium-question clinical-details-step"><span className="question-number">10</span><h3>Complete the last clinical details</h3><p className="muted">Enter the value if known. If you do not know it, type <b>Not known</b>. The report upload below remains optional.</p><div className="grid grid-3"><F label="HbA1c % *" v={o.hba1c||''} set={v=>setO({...o,hba1c:v})}/><F label="Systolic BP *" v={o.systolicBP||''} set={v=>setO({...o,systolicBP:v})}/><F label="Cholesterol *" v={o.cholesterol||''} set={v=>setO({...o,cholesterol:v})}/></div><div className="field"><label>Current treatment / medicines *</label><textarea className="textarea" placeholder="List medicines/treatment, or type None / Not known" value={o.currentTreatment} onChange={e=>setO({...o,currentTreatment:e.target.value})}/></div><div className="optional-report-upload truly-optional"><div><Upload size={20}/><span><b>Previous medical report <em>Optional</em></b><small>Skip this if you do not have a report. PDF or image accepted.</small></span></div><input multiple type="file" accept="image/*,.pdf" onChange={e=>upload(e.target.files)}/>{o.uploadedReports.length>0&&<strong>{o.uploadedReports.length} file(s) attached</strong>}</div></div>}
    {validation&&<div className="error onboarding-error">{validation}</div>}<div className="modal-actions onboarding-actions">{step>0&&<button className="btn ghost" onClick={()=>{setStep(step-1);setValidation('')}}>Back</button>}<div className="mandatory-note">Complete the health questions to continue. The report upload can be left empty.</div>{step<qs.length?<button disabled={!(o as any)[q[0]]} className="btn primary" onClick={()=>{if(!(o as any)[q[0]]){setValidation('Choose an answer before continuing.');return}setStep(step+1);setValidation('')}}>Save & continue</button>:<button disabled={!finalReady} className="btn primary" onClick={finish}>Calculate screening urgency & open camera</button>}</div></div></div>
}
...
export function PatientEyeCapture({p}:{p:PatientProfile}){
  ...
  const[eyeResults,setEyeResults]=useState<Partial<Record<EyeSide,{image:string;quality:CaptureQuality;scan:PixelScan;report:UploadRecord}>>>({});
  ...
  const[fundusPair,setFundusPair]=useState<{right:UploadRecord|null;left:UploadRecord|null}>({right:null,left:null});
  ...
  const[preliminaryReport,setPreliminaryReport]=useState<UploadRecord|null>(()=>readDB().reports.filter(r=>r.patientId===p.id&&r.category==='preliminary-screening'&&(!r.eyeSide||r.eyeSide==='right')).sort((a,b)=>b.uploadedAt.localeCompare(a.uploadedAt))[0]||null);
  ...
  function saveGeneratedPdf(doc:any,category:'preliminary-screening'|'ai-screening',label:string,side?:EyeSide){
    const dataUrl=doc.output('datauristring');
    const sideLabel=side?`${side==='left'?'Left':'Right'}-Eye-`:'';
    const report:UploadRecord={id:uid('report'),name:`DRISHTI-AI-${sideLabel}${label}-${p.name.replace(/\s+/g,'-')}-${new Date().toISOString().slice(0,10)}.pdf`,type:'application/pdf',size:Math.round(dataUrl.length*.75),dataUrl,uploadedAt:new Date().toISOString(),source:'camera',patientId:p.id,category,generated:true,eyeSide:side};
    const db=readDB();db.reports=db.reports.filter(r=>!(r.patientId===p.id&&r.category===category&&(side? r.eyeSide===side : true)));db.reports.unshift(report);writeDB(db);return report;
  }
  ...
  function createPreliminaryReport(image:string,quality:CaptureQuality,scan:PixelScan|undefined,side:EyeSide){
    ...
    row('Patient',p.name);row('Eye photographed',side==='left'?'Left eye':'Right eye');row('Diabetes status',o?.diabetesStatus||'Not provided');row('Duration',o?.duration||'Not provided');row('HbA1c',o?.hba1c||'Not provided');row('Systolic BP',o?.systolicBP||'Not provided');row('Previous DR',o?.drHistory||'Not provided');row('Vision symptoms',o?.symptoms||'Not provided');
  ...
  async function start(){
    stopStream();if(raf.current!==null){cancelAnimationFrame(raf.current);raf.current=null}countdownToken.current+=1;stableSince.current=null;stableAnchor.current=null;lockAnchor.current=null;eyeBox.current=null;lastFrameTime.current=-1;setCaptured('');setCapturedQuality(null);setPixelScan(null);setPredictionReady(false);setScanRejected(false);setAi(null);setCount(null);setReady(false);setReadiness(0);setProcessingStep(-1);setScanProgress(0);setPhaseSafe('starting');await ensureAudio();
    ...
    try{
      ...
      setPhaseSafe('scanning');setMode('loading');fallbackAt.current=performance.now()+3200;setGuide(`Camera ready for ${eyeSide==='left'?'LEFT':'RIGHT'} eye. Loading advanced eye detector…`);void tone(720,.10,.26,'sine');scanLoop();
      initializeMediaPipe().then(()=>setGuide(`${eyeSide==='left'?'LEFT':'RIGHT'} eye detector ready. Bring that eye inside the oval and hold it still.`)).catch(err=>{console.warn('MediaPipe unavailable; manual guided mode active.',err);detector.current=null;setMode('guided');setReady(false);setGuide('Automatic eye tracking is unavailable. Use the oval guide and Manual capture; auto-capture stays disabled for safety.');});
  ...
  function eyeBoxFromLandmarks(lm:any[]):EyeBox{
    // MediaPipe anatomical-eye landmark sets: subject's right eye uses 33/133, left uses 362/263.
    const ids=eyeSide==='right'?[33,133,159,145,153,154,155,173]:[362,263,386,374,380,381,382,398];
    const pts=ids.map(i=>lm[i]).filter(Boolean);const xs=pts.map((q:any)=>q.x),ys=pts.map((q:any)=>q.y);const x1=Math.min(...xs),x2=Math.max(...xs),y1=Math.min(...ys),y2=Math.max(...ys);return{x:x1,y:y1,w:x2-x1,h:y2-y1,cx:(x1+x2)/2,cy:(y1+y2)/2};
  }

  function evaluateFrame(v:HTMLVideoElement){
    const q=frameMetrics(v);let isReady=false,score=0,msg=`Place your ${eyeSide==='left'?'LEFT':'RIGHT'} eye inside the oval.`,size=0,box:EyeBox|null=null;
    ...
    if(detectorModeRef.current==='mediapipe'&&detector.current){
      ...
      else{eyeBox.current=null;msg=`${eyeSide==='left'?'LEFT':'RIGHT'} eye not detected. Re-centre that eye and move closer.`;score=8;}
    }else{
  ...
  async function captureFrame(auto=false){
    ...
    c.width=820;c.height=Math.max(340,Math.round(820*(cropH/cropW)));const ctx=c.getContext('2d')!;ctx.save();ctx.translate(c.width,0);ctx.scale(-1,1);ctx.drawImage(v,sx,sy,cropW,cropH,0,0,c.width,c.height);ctx.restore();const image=c.toDataURL('image/jpeg',.96);setCaptured(image);setUploadError('');setCount(null);setReady(false);setPhaseSafe('captured');stopStream();if(raf.current!==null){cancelAnimationFrame(raf.current);raf.current=null}await shutterSound();setGuide(`${eyeSide==='left'?'Left':'Right'} eye capture complete. Running image scan…`);void runPostCapture(image,eyeSide);
  }
  ...
  async function runPostCapture(image:string,side:EyeSide){
    setPhaseSafe('processing');setPredictionReady(false);setScanRejected(false);setScanProgress(8);setProcessingStep(0);
    try{
      const scan=await analyzeCapturedPixels(image);setPixelScan(scan);setCapturedQuality(scan.quality);setScanProgress(42);
      setProcessingStep(1);const freshRisk=assessScreeningUrgency(p.onboarding||defaults);if(p.onboarding)updatePatient(p.userId,{onboarding:{...p.onboarding,risk:freshRisk}});setScanProgress(66);
      setProcessingStep(2);
      if(scan.quality.label==='Retake recommended'){
        setScanProgress(100);setScanRejected(true);setPredictionReady(false);setProcessingStep(4);setPhaseSafe('captured');setGuide(`${side==='left'?'Left':'Right'} eye image quality is too low. Please retake before screening guidance is generated.`);await tone(320,.18,.34,'sine');setTimeout(()=>predictionRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),120);return;
      }
      setScanProgress(82);setProcessingStep(3);const report=createPreliminaryReport(image,scan.quality,scan,side);setPreliminaryReport(report);setEyeResults(prev=>({...prev,[side]:{image,quality:scan.quality,scan,report}}));setScanProgress(100);setPredictionReady(true);setProcessingStep(4);setPhaseSafe('captured');await tone(920,.16,.38,'sine');setTimeout(()=>predictionRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),120);
    }catch(e:any){setPhaseSafe('error');setUploadError(e?.message||'Captured eye image could not be analyzed.');}
  }

  function selectEye(side:EyeSide){
    if(side===eyeSide)return;stop();setEyeSide(side);setUploadError('');setCount(null);setReady(false);setReadiness(0);const existing=eyeResults[side];if(existing){setCaptured(existing.image);setCapturedQuality(existing.quality);setPixelScan(existing.scan);setPreliminaryReport(existing.report);setPredictionReady(true);setProcessingStep(4);setScanProgress(100);setPhaseSafe('captured');setGuide(`${side==='left'?'Left':'Right'} eye result loaded. Retake or upload a new photo if needed.`);}else{setCaptured('');setCapturedQuality(null);setPixelScan(null);setPredictionReady(false);setScanRejected(false);setProcessingStep(-1);setScanProgress(0);setPreliminaryReport(null);setPhaseSafe('idle');setGuide(`Selected ${side==='left'?'LEFT':'RIGHT'} eye. Choose camera or upload an eye photo.`);}
  }

  async function uploadEyePhoto(file?:File){
    if(!file)return;const side=eyeSide;setUploadError('');if(!file.type.startsWith('image/')){setUploadError('Please upload an eye image (JPG, PNG, HEIC/WebP if your browser supports it).');return}if(file.size>8*1024*1024){setUploadError('Eye image is larger than 8 MB. Please use a smaller image.');return}
    try{stop();setInputMode('upload');setPhaseSafe('processing');const image=await new Promise<string>((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result));r.onerror=()=>reject(r.error);r.readAsDataURL(file)});setCaptured(image);setGuide(`${side==='left'?'Left':'Right'} eye photo uploaded. Running the same image-quality scan…`);void runPostCapture(image,side)}catch(e:any){setPhaseSafe('error');setUploadError(e?.message||'Eye photo could not be read.');}
  }


  async function pickFundus(side:'right'|'left',f?:File){if(!f)return;const rec=await toUpload(f,'fundus');setFundusPair(prev=>({...prev,[side]:rec}));setAi(null);setAiReport(null)}
  async function analyze(){
    const right=fundusPair.right,left=fundusPair.left;if(!right||!left)return;
    setBusy(true);
    try{
      const [rq,lq]=await Promise.all([assessFundusQuality(right),assessFundusQuality(left)]);
      if(rq.quality==='unusable'||lq.quality==='unusable')throw new Error('One or both retinal images are ungradable. Please replace the affected fundus image before analysis.');
      const result=await runTrainedDRModel({id:uid('patient-preview'),patientId:p.id,phcId:'patient-preview',createdAt:new Date().toISOString(),status:'draft',right:{eye:'right',image:right,quality:rq.quality,qualityScore:rq.score,qualityGuidance:rq.guidance,qualityMethod:rq.decisionSource,observations:{}},left:{eye:'left',image:left,quality:lq.quality,qualityScore:lq.score,qualityGuidance:lq.guidance,qualityMethod:lq.decisionSource,observations:{}}});
      setAi(result);setAiReport(createAIReport(result,right.dataUrl,left.dataUrl));
    }catch(e:any){alert(e.message||'Trained retinal model service is unavailable. Continue to PHC booking for fundus imaging.')}finally{setBusy(false)}
  }
  ...
  return <>
    <PageTitle title="Left / Right eye capture" sub="Select the eye first, then use the advanced camera or upload an eye photo. Each eye is kept separate in the result and report."/>
    ...
    <section className="eye-acquisition-panel">
      <div className="eye-selection-head"><div><span className="eyebrow">Step 2 · Eye selection</span><h2>Which eye are you checking?</h2><p>Select the anatomical eye. Camera tracking and the generated report will stay tied to this selection.</p></div><div className="eye-completion-summary"><span className={eyeResults.right?'done':''}>Right {eyeResults.right?'✓':'—'}</span><span className={eyeResults.left?'done':''}>Left {eyeResults.left?'✓':'—'}</span></div></div>
      <div className="eye-side-switch"><button type="button" className={eyeSide==='right'?'selected':''} onClick={()=>selectEye('right')}><span>R</span><b>Right Eye</b><small>{eyeResults.right?'Captured · tap to review':'Capture or upload'}</small></button><button type="button" className={eyeSide==='left'?'selected':''} onClick={()=>selectEye('left')}><span>L</span><b>Left Eye</b><small>{eyeResults.left?'Captured · tap to review':'Capture or upload'}</small></button></div>
      <div className="eye-source-switch"><button type="button" className={inputMode==='camera'?'selected':''} onClick={()=>{setInputMode('camera');setUploadError('')}}><Camera size={17}/>Use Camera</button><button type="button" className={inputMode==='upload'?'selected':''} onClick={()=>{stop();setInputMode('upload')}}><Upload size={17}/>Upload Eye Photo</button></div>
      {inputMode==='upload'&&<label className="eye-photo-upload"><Upload size={26}/><div><b>Upload {eyeSide==='left'?'LEFT':'RIGHT'} eye photo</b><span>Use a close, sharp photo showing one eye. JPG/PNG/WebP, up to 8 MB.</span></div><input type="file" accept="image/*" onChange={e=>void uploadEyePhoto(e.target.files?.[0])}/></label>}
      {uploadError&&<div className="error">{uploadError}</div>}
    </section>

    <div className="capture-layout advanced-capture-layout">
      <section className="capture-stage-card advanced-camera-card">
        <div className="capture-head"><div><span className="eyebrow">Advanced camera · {eyeSide==='left'?'LEFT':'RIGHT'} eye</span><h2>Align only the selected eye inside the guide</h2><p className="muted">Green lock is continuously verified. If the eye moves during the countdown, capture is cancelled automatically.</p></div><span className={`detector-pill ${detectorMode}`}>{eyeSide==='left'?'LEFT':'RIGHT'} · {detectorMode==='mediapipe'?'Eye landmarks active':detectorMode==='guided'?'Manual guided mode':'Starting detector'}</span></div>
        {inputMode==='camera'?<div className={`live-capture premium advanced ${ready?'ready':''} ${phase==='countdown'?'counting':''}`}>
          <video ref={video} muted playsInline/>
          <div className="capture-mask"><div className="eye-guide advanced"><span/><em>{eyeSide==='left'?'LEFT EYE':'RIGHT EYE'}</em></div></div>
          ...
          {phase==='captured'&&processingStep<0&&<div className="capture-success"><CheckCircle2 size={42}/><b>{eyeSide==='left'?'Left':'Right'} eye captured</b></div>}
        </div>:<div className="upload-eye-stage">{captured?<><img src={captured} alt={`${eyeSide} eye upload`}/><div className="upload-eye-stage-copy"><CheckCircle2 size={28}/><b>{eyeSide==='left'?'Left':'Right'} eye photo ready</b><span>Image is being checked for usable quality, glare, brightness, contrast and detail.</span></div></>:<><Upload size={40}/><b>Upload {eyeSide==='left'?'LEFT':'RIGHT'} eye photo</b><span>Choose a close-up image showing the selected eye clearly.</span></>}</div>}
        <canvas ref={canvas} style={{display:'none'}}/>
        {inputMode==='camera'&&<><div className="readiness-track"><div style={{width:`${readiness}%`}}/></div><div className="capture-metrics"><span>Light {Math.round(metrics.brightness)}</span><span>Contrast {Math.round(metrics.contrast)}</span><span>Detail {Math.round(metrics.sharpness)}</span><span>{detectorMode==='mediapipe'?'Continuous eye tracking':'Auto-capture unavailable'}</span></div></>}
        <div className="capture-actions">
          {inputMode==='camera'&&<>{phase==='idle'||phase==='error'||phase==='captured'?<button className="btn primary" onClick={start}><Camera size={17}/>{captured?'Retake selected eye':'Start advanced camera'}</button>:<button className="btn ghost" onClick={stop}>Stop camera</button>}<button className="btn soft" disabled={!streamRef.current||phase==='countdown'||phase==='processing'} onClick={()=>void captureFrame(false)}>Manual capture</button><button className="btn ghost" onClick={()=>void testBuzzer()}>{soundReady?'Test loud buzzer':'Enable / test sound'}</button></>}
          {inputMode==='upload'&&<label className="btn primary upload-as-button"><Upload size={17}/>Choose {eyeSide==='left'?'left':'right'} eye photo<input type="file" accept="image/*" onChange={e=>void uploadEyePhoto(e.target.files?.[0])}/></label>}
        </div>
        {inputMode==='camera'&&<div className={`sound-status ${soundReady?'ok':'warn'}`}><b>{soundReady?'Sound unlocked':'Sound not yet confirmed'}</b><span>{soundReady?'Buzzer, countdown tones and shutter effect are enabled.':'Tap “Enable / test sound”. Also check browser tab/system volume.'}</span></div>}
        {captured&&<div className="captured-result eye-only-result"><div><span className="eyebrow">{eyeSide==='left'?'Left eye':'Right eye'} · saved image</span><h3>{inputMode==='upload'?'Eye photo uploaded':'Eye-only image captured'}</h3><p>This crop is actually scanned for brightness, contrast, detail and glare. It is not used to claim retinal lesion detection.</p></div><img src={captured} alt="Captured eye crop"/></div>}
      </section>
    ...
    <div ref={predictionRef} className={`prediction-stage ${predictionReady?'visible':''}`}>
      ...
      {predictionReady&&<div className="screening-result-grid">
        <div className={`clinical-risk-panel ${riskBand}`}>
          ...
          <div><span className="eyebrow">Preliminary triage index</span><h2>{riskBand.toUpperCase()}</h2><p>{nextActionCopy()}</p><small>Not a disease probability. Q&A determines screening urgency; this selected-eye photo contributes image usability/quality only, not retinal DR grading.</small></div>
        </div>
        <div className="quality-result-card"><span className="eyebrow">{eyeSide==='left'?'Left eye':'Right eye'} image-quality scan</span><h3>{capturedQuality?.label||'Completed'}</h3><strong>{capturedQuality?.score||0}/100</strong>{capturedQuality?.guidance.map(x=><p key={x}>{x}</p>)}{pixelScan&&<small>Brightness {Math.round(pixelScan.brightness)} · contrast {Math.round(pixelScan.contrast)} · detail {Math.round(pixelScan.sharpness)} · glare {Math.round(pixelScan.glareRatio*100)}%</small>}</div>
      </div>}
      ...
      {predictionReady&&preliminaryReport&&<div className="preliminary-report-ready"><div className="report-ready-icon"><FileText size={24}/></div><div><span className="eyebrow">Report generated</span><h3>{eyeSide==='left'?'Left Eye':'Right Eye'} Preliminary Risk & Screening Report</h3><p>Includes Q&A risk context, real captured-image quality metrics, triage chart, next-check interval / PHC recommendation and safety notes.</p><small>{new Date(preliminaryReport.uploadedAt).toLocaleString()} · Saved in Reports</small></div><div className="report-ready-actions">{preliminaryReport.dataUrl&&<a className="btn primary" href={preliminaryReport.dataUrl} download={preliminaryReport.name}><FileDown size={16}/>Download report</a>}<Link className="btn ghost" to="/patient/reports">Open Reports</Link></div></div>}

      {predictionReady&&<div className="card fundus-next true-fundus-panel"><div><span className="eyebrow">Already have real fundus images?</span><h3>Run bilateral trained retinal screening</h3><p className="muted">Upload the actual RIGHT and LEFT retinal/fundus images separately. DRISHTI never duplicates one eye image into both eyes. Each image passes a real quality check before the trained retinal model is called.</p></div><div className="fundus-pair-grid"><label className={`fundus-upload-card ${fundusPair.right?'loaded':''}`}><span><b>Right-eye fundus</b><small>{fundusPair.right?.name||'Choose retinal image'}</small></span><Upload size={20}/><input type="file" accept="image/*" onChange={e=>void pickFundus('right',e.target.files?.[0])}/>{fundusPair.right?.dataUrl&&<img src={fundusPair.right.dataUrl} alt="Right fundus preview"/>}</label><label className={`fundus-upload-card ${fundusPair.left?'loaded':''}`}><span><b>Left-eye fundus</b><small>{fundusPair.left?.name||'Choose retinal image'}</small></span><Upload size={20}/><input type="file" accept="image/*" onChange={e=>void pickFundus('left',e.target.files?.[0])}/>{fundusPair.left?.dataUrl&&<img src={fundusPair.left.dataUrl} alt="Left fundus preview"/>}</label></div><div className="truth-note"><ShieldAlert size={16}/><span><b>Real analysis path:</b> bilateral image quality → trained EfficientNet DR inference → per-eye grade/confidence → ophthalmologist review.</span></div><button className="btn primary fundus-analyze-btn" disabled={!fundusPair.right||!fundusPair.left||busy} onClick={analyze}>{busy?'Checking quality & running trained model…':'Analyze both fundus images'}</button></div>}
