...
function Onboarding({p,onDone}:{p:PatientProfile;onDone:()=>void}){
  ...
  return <div className="modal-back onboarding-backdrop"><div className="modal onboarding premium-onboarding"><div className="onboarding-top"><div><span className="eyebrow">Mandatory patient health profile</span><h2>{step<qs.length?`Question ${step+1} of ${qs.length}`:'Final clinical details'}</h2><p className="muted">All questions are required. Only uploading a previous report is optional.</p></div><div className="required-badge"><ShieldAlert size={16}/>Required</div></div><div className="progress premium-progress"><div style={{width:`${progress}%`}}/></div>
    {step<qs.length?<div className="question premium-question"><span className="question-number">{String(step+1).padStart(2,'0')}</span><h3>{q[1]}</h3><div className="option-grid premium-options">{q[2].map(v=><button type="button" key={v} className={'option '+((o as any)[q[0]]===v?'selected':'')} onClick={()=>{setO({...o,[q[0]]:v});setValidation('')}}>{v}</button>)}</div></div>:<div className="question premium-question clinical-details-step"><span className="question-number">10</span><h3>Complete the last clinical details</h3><p className="muted">Enter the value if known. If you do not know it, type <b>Not known</b>. The report upload below remains optional.</p><div className="grid grid-3"><F label="HbA1c % *" v={o.hba1c||''} set={v=>setO({...o,hba1c:v})}/><F label="Systolic BP *" v={o.systolicBP||''} set={v=>setO({...o,systolicBP:v})}/><F label="Cholesterol *" v={o.cholesterol||''} set={v=>setO({...o,cholesterol:v})}/></div><div className="field"><label>Current treatment / medicines *</label><textarea className="textarea" placeholder="List medicines/treatment, or type None / Not known" value={o.currentTreatment} onChange={e=>setO({...o,currentTreatment:e.target.value})}/></div><div className="optional-report-upload"><div><Upload size={20}/><span><b>Previous medical report</b><small>Optional — PDF or image</small></span></div><input multiple type="file" accept="image/*,.pdf" onChange={e=>upload(e.target.files)}/>{o.uploadedReports.length>0&&<strong>{o.uploadedReports.length} file(s) attached</strong>}</div></div>}
    {validation&&<div className="error onboarding-error">{validation}</div>}<div className="modal-actions onboarding-actions">{step>0&&<button className="btn ghost" onClick={()=>{setStep(step-1);setValidation('')}}>Back</button>}<div className="mandatory-note">No skip — complete all required questions to continue.</div>{step<qs.length?<button disabled={!(o as any)[q[0]]} className="btn primary" onClick={()=>{if(!(o as any)[q[0]]){setValidation('Choose an answer before continuing.');return}setStep(step+1);setValidation('')}}>Save & continue</button>:<button disabled={!finalReady} className="btn primary" onClick={finish}>Calculate screening urgency & open camera</button>}</div></div></div>
}
...
export function PatientEyeCapture({p}:{p:PatientProfile}){
  ...
  async function runPostCapture(image:string,side:EyeSide){
    setPhaseSafe('processing');setPredictionReady(false);setScanRejected(false);setScanProgress(6);setProcessingStep(0);
    await sleep(850);setScanProgress(18);const scan=await analyzeCapturedPixels(image);setPixelScan(scan);setCapturedQuality(scan.quality);
    await sleep(900);setScanProgress(38);setProcessingStep(1);const freshRisk=assessScreeningUrgency(p.onboarding||defaults);if(JSON.stringify(freshRisk)!==JSON.stringify(p.onboarding?.risk)&&p.onboarding)updatePatient(p.userId,{onboarding:{...p.onboarding,risk:freshRisk}});
    await sleep(1050);setScanProgress(61);setProcessingStep(2);await sleep(1050);setScanProgress(82);setProcessingStep(3);await sleep(950);
    if(scan.quality.label==='Retake recommended'){
      setScanProgress(100);await sleep(650);setScanRejected(true);setPredictionReady(false);setProcessingStep(4);setPhaseSafe('captured');setGuide(`${side==='left'?'Left':'Right'} eye image quality is too low. Please retake before screening guidance is generated.`);await tone(320,.18,.34,'sine');setTimeout(()=>predictionRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),240);return;
    }
    const report=createPreliminaryReport(image,scan.quality,scan,side);setPreliminaryReport(report);setEyeResults(prev=>({...prev,[side]:{image,quality:scan.quality,scan,report}}));setScanProgress(100);await sleep(700);setPredictionReady(true);setProcessingStep(4);setPhaseSafe('captured');await tone(920,.16,.38,'sine');setTimeout(()=>predictionRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),240);
  }
